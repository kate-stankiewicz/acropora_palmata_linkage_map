...
for(i in chrom){
  ...
  for(j in mg){
    ...
    dosage_chr<-read.table(paste0(gen,"/rr__",j,"_",i,".ps21.txt"), header = F)
    ...
    #write out the plot
    g5 <- ggplot(hy_um_chr, aes(x = (pos/1000000), y = value, color = species)) +
    ...
    pdf(paste0(gen,"/combined_plots/all_",j,"_",i,".pdf"), paper = "a4r", width = 0, height = 0)
    print(ggarrange(g2 + rremove("x.title"),g6,nrow = 2, align="v"))
    dev.off()
  }
}
